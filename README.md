...
You will need <a href="https://github.com/Piotrekol/StreamCompanion">StreamCompanion</a> in order for `!np` and `!nppp` to work. This bot uses localhost:20727 to get the map's information provided by this amazing pp counter.

## Account and Authorization

You will need to fill in some fields in a `.env` file within this folder. Just make a text file within this folder and name it exactly `.env`.
...
The bot will also need your broadcaster ID to be able to do some things on your channel, otherwise it doesn't know what channel to read your mods list (!polls), read your followers list (!followage),... from. You can run the `GetBroadcasterId.exe` for that, since it's fully automated now!
...
For safety reasons, Twitch API (and many others) uses access tokens to check whether you're allowed to make an API call. Some technical stuff, but it basically means you need a "key" to be allowed to access certain "rooms". I compacted all the different URL's into 1 big scope. This scope includes:

- Read your chat
- Send messages in your chat
- Read your mods list
...
**If you want to help working on the bot:**

```
pip install -r requirements.txt
```

**If you want to help improve the website:**
